...
from archs import Sender, Receiver, ReinforcedReceiver
from features import OneHotLoader, UniformLoader
from games import NoisySymbolGameGS, SymbolGameGS
from archs import BSChannel

currentdir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
parentdir = os.path.dirname(currentdir)
sys.path.insert(0, parentdir)

from intervention import CallbackEvaluator
...
def main(params):
    ...
    if not opts.variable_length:
        ...
        if opts.mode == 'gs':
            ...
            game = SymbolGameGS(sender, receiver, diff_loss)
        elif opts.mode == 'gs_noisy':
            sender = core.GumbelSoftmaxWrapper(
                agent=sender, temperature=opts.temperature)
            receiver = Receiver(n_bits=opts.n_bits,
                                n_hidden=opts.receiver_hidden)
            receiver = core.SymbolReceiverWrapper(
                receiver, vocab_size=opts.vocab_size, agent_input_size=opts.receiver_hidden)
            channel = BSChannel(0.05)
            game = NoisySymbolGameGS(sender, receiver, channel, diff_loss)
        elif opts.mode == 'rf':
    ...
    intervention = CallbackEvaluator(test_loader,
                                     device=device,
                                     is_gs=opts.mode in ['gs', 'gs_noisy'],
                                     loss=loss,
